...
export const weeklyPlansService = {
  ...
  async listAllItems(userId: string): Promise<WeeklyPlanItem[]> {
    if (!isSupabaseConfigured || !supabase) return localDb.listAllWeeklyPlanItems(userId);

    const { data: plans, error: planError } = await supabase.from("weekly_plans").select("id").eq("user_id", userId);
